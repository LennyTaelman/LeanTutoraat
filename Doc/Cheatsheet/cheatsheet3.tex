\documentclass[10pt,landscape]{article}
\usepackage[utf8]{inputenc}
\usepackage[landscape,margin=0.5in]{geometry}
\usepackage{tabularx}
\usepackage{booktabs}
\usepackage{fancyvrb}
\usepackage{amsfonts}
\usepackage{multicol}

\DefineVerbatimEnvironment{code}{Verbatim}{fontsize=\small}

\title{Lean Tutoraat cheat sheet}
\author{}
\date{}

\begin{document}

% \maketitle

% \section*{Tactics}

\begin{table}[h!]
\centering
\begin{tabularx}{\textwidth}{l p{8.5cm} X}
\toprule
\textbf{\!Tactic} & \textbf{\!Description} & \textbf{\!Example} \\
\midrule

\texttt{rfl} & 
Prove equalities that hold \emph{by definition}. &
\parbox[t]{8cm}{\ttfamily\small
example :\ 1 = 1 := by\\\hspace*{1em}rfl
} \\

\midrule

\texttt{simp} &
Simplifies trivial expressions such as \texttt{a + 0}, \texttt{1 * a} or \texttt{0 * a}. &
\parbox[t]{12cm}{\ttfamily\small
example (a : $\mathbb{Z}$) : 1 * a + 0 = a := by\\\hspace*{1em}simp
} \\

\midrule

\texttt{numbers} & 
Prove (in)equalities between purely numerical expressions. &
\parbox[t]{12cm}{\ttfamily\small
example :\ 5 \^{} 3 < 2 \^{} 7 := by\\\hspace*{1em}numbers
} \\

\midrule

\texttt{algebra} & 
Prove algebraic identities. &
\parbox[t]{12cm}{\ttfamily\small
example (x y :\ $\mathbb{R}$) :\ (x + y) * (x - y) = x \^{} 2 - y \^{} 2 := by\\\hspace*{1em}algebra
} \\ 

\midrule

\texttt{rewrite [h]}, \quad \texttt{rewrite [$\leftarrow$h]} & 
If hypothesis \texttt{h} is of the form \texttt{a = b}, replace \texttt{a} with
\texttt{b} in the goal. The version with $\leftarrow$h replaces \texttt{b} with \texttt{a}. &
\parbox[t]{12cm}{\ttfamily\small
example (x :\ $\mathbb{Q}$) (h :\ x = 2) :\ x \^{} 2 = 4 := by\\
\hspace*{1em}rewrite [h];\ numbers
} \\ 

\midrule

\texttt{positivity} & 
Prove goals of the form \texttt{a > 0} or \texttt{a $\geq$ 0}. &
\parbox[t]{12cm}{\ttfamily\small
example (x :\ $\mathbb{R}$) :\ x \^{} 2 $\geq$ 0 := by\\\hspace*{1em}positivity
} \\ 

\midrule

\texttt{extra} & 
Prove inequalities of the form \texttt{a + e > a} or \texttt{a + e $\geq$ a},
where \texttt{e}\ is (strictly) positive. &
\parbox[t]{12cm}{\ttfamily\small
example (a b :\ $\mathbb{Z}$) (h :\ a $\geq$ 0) :\ a + b $\geq$ b := by\\\hspace*{1em}extra
} \\ 

\midrule

\texttt{rel [h]} & 
Use inequality \texttt{h} to prove a directly related inequality. &
\parbox[t]{12cm}{\ttfamily\small
example (a b :\ $\mathbb{R}$) (h :\ a $\geq$ b) :\ a + 1 $\geq$ b + 1 := by\\
\hspace*{1em}rel [h]
} \\ 

\midrule

\texttt{linarith} & 
A powerful tactic to automatically prove linear inequalities from linear (in)equalities 
in the hypotheses. &
\parbox[t]{12cm}{\ttfamily\small
example (a b :\ $\mathbb{R}$) (h1 :\ a > 2) (h2 :\ a + b < 3) :\ b < 1 := by\\
\hspace*{1em}linarith
} \\ 


\midrule

\texttt{calc} & 
Chain (in)equalities together to prove an (in)equality. &
\parbox[t]{12cm}{\ttfamily\small
example (x y :\ $\mathbb{R}$) :\ x \^{} 2 + y \^{} 2 - 2 * x * y $\geq$ 0 := by\\
\hspace*{1em}calc\\
\hspace*{2em}x \^{} 2 + y \^{} 2 - 2 * x * y = (x - y) \^{} 2 := by algebra\\
\hspace*{15em}\_ $\geq$ 0\hspace*{4.4em}:= by positivity
} \\ 

\midrule

\texttt{use} & 
Start proving an existential statement ($\exists$) by providing the value that
you will use. &
\parbox[t]{12cm}{\ttfamily\small
example :\ $\exists$ n :\ $\mathbb{N}$, 81 * n = 2025 := by\\
\hspace*{1em}use 25\\
\hspace*{1em}numbers
} \\ 

\midrule

\texttt{intro} & 
Start proving a universal statement ($\forall$) by introducing an arbitrary
variable. &
\parbox[t]{12cm}{\ttfamily\small
example :\ $\forall$ x :\ $\mathbb{R}$, x \^{} 2 $\geq$ 0 := by\\
\hspace*{1em}intro x\\
\hspace*{1em}positivity
} \\ 

\midrule

\texttt{have} & 
Add another hypothesis (with proof). This becomes available to use in the rest
of the proof. &
\parbox[t]{12cm}{\ttfamily\small
example (x :\ $\mathbb{R}$) (h :\ x + 1 > 2) :\ x * x$^{-1}$ = 1 := by\\
\hspace*{1em}have h2 :\ x $\neq$ 0 := by linarith\\
\hspace*{1em}algebra
} \\ 

\midrule

\texttt{simple\_induction n with k IH} & 
Prove something for all natural numbers \texttt{n} by induction. Creates a base case (\texttt{n=0}) and an inductive step (\texttt{n=k+1} with induction hypothesis \texttt{IH} for \texttt{n=k}). &
\parbox[t]{12cm}{\ttfamily\small
example (n :\ $\mathbb{N}$) :\ n $\geq$ 0 := by\\
\hspace*{1em}simple\_induction n with k IH\\
\hspace*{1em}· numbers\\
\hspace*{1em}· positivity
} \\ 

\midrule

\parbox[t]{6.5cm}{\texttt{induction\_from\_starting\_point n, h with k hk IH}} &
Prove something for all natural numbers \texttt{n $\geq$ C} by induction, given hypothesis \texttt{h : n $\geq$ C}. Creates a base case for \texttt{n=C} and an inductive step. &
\parbox[t]{12cm}{\ttfamily\small
example (n :\ $\mathbb{N}$) (h :\ n $\ge$ 3) :\ n $\ge$ 3 := by\\
\hspace*{1em}induction\_from\_starting\_point n, h with k hk IH\\
\hspace*{1em}· numbers\\
\hspace*{1em}· linarith
} \\

\bottomrule
\end{tabularx}
\end{table}

\newpage

% \section*{Special symbols}

\begin{table}[h!]
\begin{tabular}{cll}
\toprule
\textbf{Symbol} & \textbf{Type} & \textbf{Description} \\
\midrule
$\mathbb{N}$ & \texttt{\textbackslash N} & natural numbers \\
$\mathbb{Z}$ & \texttt{\textbackslash Z} & integers \\
$\mathbb{Q}$ & \texttt{\textbackslash Q} & rational numbers \\
$\mathbb{R}$ & \texttt{\textbackslash R} & real numbers \\
$\leq$ & \texttt{\textbackslash le} & less than or equal to \\
$\geq$ & \texttt{\textbackslash ge} & greater than or equal to \\
$\pi$ & \texttt{\textbackslash pi} & pi \\
$a^{-1}$ & \texttt{\textbackslash inv} & inverse \\
$\forall$ & \texttt{\textbackslash forall} & for all \\
$\exists$ & \texttt{\textbackslash exists} & exists \\
$\leftarrow$ & \texttt{\textbackslash l} & used in \texttt{rewrite [$\leftarrow$h]} \\
$\cdot$ & \texttt{\textbackslash .} & bullet (used for induction cases) \\
\bottomrule
\end{tabular}
\end{table}

\end{document}
