...
lemma fac_pos (n : ℕ) : fac n > 0 := by
  ...
  · calc
  fac (k + 1) = (k + 1) * fac k := by rewrite[fac_succ]; algebra
  _ > (k + 1) * 0 := by rel[IH]
  _ = 0 := by algebra
...
lemma fac_ge_one (n : ℕ) : fac n ≥ 1 := by
  have h1 : fac n > 0 := by apply fac_pos
...
lemma fac_strictly_monotone (n : ℕ) (h : n ≥ 1) : fac (n + 1) > fac n := by
  have h1 : fac n > 0 := by apply fac_pos
  calc
    fac (n + 1) = fac n * (n + 1) := by rewrite[fac_succ]; linarith
              _ ≥ fac n * (1 + 1) := by rel[h]
              _ = fac n * 2       := by simp
              _ > fac n           := by linarith
...
-- have h1 : n ≥ 1 := by
-- have h2 : fac (n + 1) > fac n := by apply fac_strictly_monotone; linarith
induction_from_starting_point n, h with k hk IH
· rw[fac_two]; linarith
· calc
fac (k + 1) > fac k := by apply fac_strictly_monotone; linarith
_ > 1 := by rel[IH]

lemma fac_gt_of_gt (n m : ℕ) (h1 : n ≥ m + 1) (h2 : m ≥ 1): fac n > fac m := by
induction_from_starting_point n, h1 with k hk IH
· apply fac_strictly_monotone; rel[h2]
· calc
fac (k + 1) > fac k := by apply fac_strictly_monotone; linarith
_ > fac m := by rel[IH]
...
· algebra
· have h7 : k + 1 ≥ 1 := by linarith
  calc
  fac (k + 1) = fac k * (k + 1) := by rw[fac_succ]; linarith
  _ ≥ fac n * (k+1) := by rel[IH]
  -- _ ≥ fac n * (n+1) := by rel[hk]
  _ ≥ fac n * 1 := by rel[h7]
  _ = fac n := by simp
...
· rw[fac_two]
. calc
fac (k + 1) ≥ fac k := by apply fac_ge_of_ge; linarith
_ ≥ 2 := by rel[IH]
-- fac (k + 1) = fac k * (k+1) := by rw[fac_succ]; linarith
-- _ ≥ 2 * (k+1) := by rel[IH]
-- _ ≥ 2 * 3 := by rel[hn]
-- _ ≥ 2 := by linarith
-- calc
-- fac n ≥ fac 2 := by apply fac_strictly_monotone; rel[hn]; linarith
...
theorem fac_bound (n : ℕ) (k : ℕ) (hn : n > 0) :
  ...
  · -- base case
    algebra
  · -- inductive step
    have h7 : n + k + 1 ≥ 1 := by linarith
    have h8 : n + k + 1 ≥ 2 := by linarith
    calc
    fac (n + k + 1) = (n + k + 1) * fac (n + k + 1 - 1) := by rewrite[fac_prev (n+k+1) h7]; linarith
    _ = (n + k + 1) * fac (n + k) := by linarith
    _ ≥ (n + k + 1) * (2^k * fac n) := by rel[IH]
    _ ≥ 2 * (2^k * fac n) := by rel[h8]
...
lemma pow_two_mul_fac_pos (n : ℕ) (k : ℕ) : 0 < 2 ^ k * fac n := by
  ...
  have h2 : 0 < fac n := by apply fac_pos
  calc
  0 = 0 * 0 := by linarith
  _ ≤ 2^k * 0 := by rel[h]
  _ < 2^k * fac n := by rel[h2]
...
lemma nat_inv_ne_zero (n : ℕ) (hn : n > 0) : nat_inv n ≠ 0 := by
  have h1: nat_inv n > 0 := by rewrite[nat_inv_def]; positivity
  positivity

lemma nat_inv_mul (n m : ℕ) : nat_inv (n * m) = nat_inv n * nat_inv m := by
  calc
  nat_inv (n * m) = (↑(n * m))⁻¹ := by rewrite[nat_inv]; rfl
                _ = (↑n)⁻¹ * (↑m)⁻¹ := by algebra
                _ = nat_inv n * nat_inv m := by rewrite[←nat_inv, ←nat_inv]; rfl
...
lemma nat_inv_lt (n m : ℕ) (h : n > m) (hm : m > 0) : nat_inv n < nat_inv m := by
  have h2 : (m : ℝ) < (n : ℝ) := by linarith
  have h3 : (m : ℝ) > 0 := by linarith
  rewrite[nat_inv, nat_inv]
  apply inv_lt_inv_of_lt h3 h2

lemma nat_inv_le_one (n : ℕ) (hn : n > 0) : nat_inv n ≤ 1 := by
  have h4 : (n ≥ 1) := by linarith
  rewrite[← nat_inv_one]
  apply nat_inv_le n 1 h4
  algebra

lemma nat_inv_lt_one (n : ℕ) (hn : n > 1) : nat_inv n < 1 := by
  rewrite[← nat_inv_one]
  apply nat_inv_lt n 1 hn
  algebra
...
lemma a_zero : a 0 = 1 := by
  rewrite[a_def, fac_zero, nat_inv_one]; rfl

lemma a_one : a 1 = 1 := by
  rewrite[a_def,fac_succ,fac_zero, nat_inv_one]; rfl

lemma a_two : a 2 = 1 / 2 := by
  rewrite[a_def,fac_succ, fac_succ,fac_zero, nat_inv_two]; rfl

lemma a_three : a 3 = 1 / 6 := by
  rewrite[a_def, fac_succ, fac_succ, fac_succ, fac_zero, nat_inv]; algebra

lemma a_succ (n : ℕ) : a (n + 1) = a n / (n + 1) := by
  rewrite[a_def, fac_succ, nat_inv_mul, ← a_def, nat_inv]; algebra
...
rewrite [a_def, a_def]
apply nat_inv_le
apply fac_ge_of_ge m n h
apply fac_pos

lemma a_lt (n m : ℕ) (h : n > m) (hn : m > 0) : a n < a m := by
have h1 : m ≥ 1 := by linarith
rewrite [a_def]
apply nat_inv_lt
apply fac_gt_of_gt
linarith
...
theorem a_bound (n : ℕ) (k : ℕ) (hn : n ≥ 1) :
  ...
  have h : (1/2) ^ k * (a n) = nat_inv (2 ^ k * fac n) := by
    rewrite[a_def,nat_inv,nat_inv]; algebra
...
lemma g_one : g 1 = 1 := by
  rewrite[g_succ]; rewrite[g_zero]; numbers

lemma g_two : g 2 = 3 / 2 := by
  rewrite[g_succ]; rewrite[g_one]; numbers

lemma g_three : g 3 = 7 / 4 := by
  rewrite[g_succ]; rewrite[g_two]; numbers
...
lemma g_formula (n : ℕ) : g n = 2 - 2 * (1/2) ^ n := by
  simple_induction n with k IH
  · rewrite[g_zero]; numbers
  · calc
  g (k + 1) = g k + (1/2)^k := by rewrite[g_succ k]; rfl
  _ = 2 - 2 * (1/2)^k + (1/2)^k := by rewrite[IH]; rfl
  _ = 2 - 2 * (1/2)^(k + 1) := by algebra
...
lemma s_strictly_monotone (n : ℕ) : s n < s (n + 1) := by
  have h : a n > 0 := by rel [a_pos n]
  calc
    s n < s n + a n := by extra
      _ = s (n + 1) := by rewrite [← s_succ]; algebra
...
lemma s_nonneg (n : ℕ) : s n ≥ 0 := by
  ...
  · rewrite [s_zero]
    algebra
  · calc
  s (k+1) = s k + a k := by rewrite[s_succ]; algebra
  _≥ 0 + a k := by rel[IH]
  _≥ 0 + 0 := by rel[a_pos k]
  _≥ 0 := by linarith
...
theorem s_geometric_bound (n : ℕ) (k : ℕ) (hn : n ≥ 1) :
  ...
  simple_induction k with k IH
  · calc
      s (n + 0) = s n := by algebra
      _≤ s n + 0 := by extra
      _≤ s n + (a n) * 0 := by algebra
  · calc
      s (n + k + 1) = s (n + k) + a (n + k) := by rw[s_succ]
      _≤ s n + (a n) * (g k) + a (n + k) := by rel[IH]
      _≤ s n + (a n) * (g k) + (1/2)^k * (a n) := by rel[a_bound n k hn]
      _= s n + (a n) * (g k + (1/2)^k) := by algebra
      _= s n + (a n) * (2 - 2 * (1/2)^k + (1/2)^k) := by rw[g_formula]
      _= s n + (a n) * (2 - (1/2)^k) := by algebra
      _= s n + (a n) * (2 - 2 * (1/2)^(k+1)) := by algebra
      _= s n + (a n) * (g (k+1)) := by rw[g_formula]
...
lemma s_le_of_le (n : ℕ) (m : ℕ) (h : n ≤ m) : s n ≤ s m := by
  induction_from_starting_point m, h with k hk IH
  · rfl
  · have h1: s k < s (k+1) := by rel [s_strictly_monotone k]
    calc
      s n ≤ s k := by rel [IH]
      _ ≤ s (k+1) := by linarith

lemma s_bound (n : ℕ) (k : ℕ) (hn : n ≥ 1) : s (n + k) < s n + 2 * (a n) := by
  -- remind Lean that `a n > 0`, so that `linarith`, `rel`, ... can use this
  have h : a n > 0 := a_pos n
  have h2 : g k < 2 := g_lt_2 k
  -- now finish the proof using `s_geometric_bound` and `g_lt_2`
  calc
  s (n+k) ≤ s n + (a n) * (g k) := by apply s_geometric_bound n k hn
  _ < s n + (a n) * 2 := by rel [h2]
...
lemma s_bound_variant (n m : ℕ) (hn : n ≥ 1) (hm : m ≥ n) : s m < s n + 2 * (a n) := by
  ...
  calc
  s m = s (n+k) := by rewrite[hk]; algebra
  _< s n + 2 * (a n) := by rel[s_bound n k hn] -- miauw miauw
...
theorem s_key_bound (n m : ℕ) (hn : n ≥ 1) : s m < s n + 2 * (a n) := by
  have h0 : a n > 0 := a_pos n
  have h1 : 2 * (a n) > 0 := by linarith
  by_cases hm : m ≥ n
  ...
  · -- or `¬ m ≥ n`, then we have `m < n`:
    push_neg at hm
    -- now finish the proof
    have hm2 : m ≤ n := by linarith
    calc
      s m ≤ s n := by rel [s_le_of_le m n hm2]
...
-/
lemma s_lt_three (n : ℕ) : s n < 3 := by
  by_cases hn : n ≥ 1
  · calc
      s n < s 1 + 2 * (a 1) := by apply s_key_bound 1 n (by rfl)
      _= 1 + 2 * (a 1) := by rw[s_one]
      _= 1 + 2 * 1 := by rw[a_one]
      _= 3 := by algebra
...
have h88: 8>0 := by numbers
have h8 : isRat 8⁻¹:= by apply isRat_inv_nat 8 h88
...
have h1: x^2 = x*x:= by algebra
rewrite[h1];apply isInt_mul h h
...
induction_from_starting_point m, h with k hk IH
· rewrite[fac_mul_a_eq_one]; apply isInt_one
...
-- first prove the identity `h2`
    rewrite[fac_succ]; algebra
  -- now finish the proof of the induction step.
  have h20 : isInt (k):= by apply isInt_nat k
  have hk : isInt (k+1) := by apply isInt_add h20 isInt_one
  have h6 : ↑(k+1) =(k : ℝ)+1 := by algebra
  rewrite[h2,h6]; apply isInt_mul hk IH
...
-/
theorem isInt_fac_mul_s (n : ℕ) (m : ℕ) (h : m + 1 ≥ n): isInt ((fac m) * s n) := by
  simple_induction n with k IH
  · rewrite[s_zero];
    have h400: ((fac m):ℝ )*0 = 0:= by algebra
    rewrite[h400];apply isInt_zero
  · -- let's first relate `(fac m) * s (k + 1)` and `(fac m) * s k`
    ...
    have h300: m ≥ k := by
      ...
      _= k := by algebra
    have h500: m +1 ≥ k := by
      calc m+1 ≥ k + 1:= by rel[h300]
      _≥ k := by algebra
    have h600: isInt (fac m * a k) := by apply isInt_fac_mul_a k m h300
    have IHH: isInt (↑(fac m) * s k):= by apply IH h500
    rewrite[h2];apply isInt_add IHH (h600)
...
lemma fac_mul_isInt_of_isRat (x : ℝ) (h : isRat x) :
  ...
  · -- prove that `(fac n) * x` is an integer
    have h3c: n≥ 1:=by linarith
    have h3: fac n= fac (n-1) * n := by rewrite[fac_prev n h3c]; rfl

    have h4: (fac n)*x= n*x *(fac (n-1)):= by rewrite[h3]; algebra
    have h5: isInt (fac (n-1)):= by apply isInt_nat (fac (n-1))
    rewrite[h4]
    apply isInt_mul h2 h5
...
lemma fac_mul_t_succ_pos (n : ℕ) : (fac n) * (t (n + 1)) > 0 := by
  have h1 : fac n ≥ 1 := by apply fac_pos
  have h2 : t (n+1) > 0 := by apply t_pos
  positivity

lemma bound_1 (n : ℕ) (hn : n ≥ 1) :
    (fac n) * t (n + 1) ≤ 2 * (fac n) * a (n + 1) := by
  have h0: n + 1 ≥ 1 := by extra
  have h1: t (n + 1) ≤ 2 * a (n+1) := by apply t_le_twice_a (n + 1) h0
  have h2: fac n > 0 := by apply fac_pos
  have h3: (2 * (fac n)) * a (n + 1) = (fac n) * (2 * a (n + 1) ) := by algebra
  rewrite[h3]
  rel[h1]
...
lemma aux_1 (n : ℕ) :
    (fac n) * (a (n + 1)) = 1 / (n + 1) := by
      simple_induction n with k IH
      · rw[a_succ, a_zero, fac_zero]; numbers
      · calc
        (fac (k+1)) * (a (k + 1 + 1)) = ((k+1) * fac k) * (a (k + 1) / (k + 1 + 1)) := by rw[fac_succ, a_succ]; algebra
        _ = fac k * a (k + 1) * (k + 1 ) / (k + 1 + 1) := by algebra
        _ = 1/(k+1) * (k + 1 ) / (k + 1 + 1) := by rw[IH]
        _ = 1 / (k + 1 + 1) := by algebra
...
lemma fac_mul_t_succ_not_integral (n : ℕ) (hn : n ≥ 2) :
  ...
  have h1 : (fac n) * t (n + 1) < 1 := by
    apply fac_mul_t_succ_lt_one n hn
  have h2 : (fac n) * t (n + 1) > 0 := by
    apply fac_mul_t_succ_pos
...
-- proof by contradiction:
  ...
  have hs : isInt ((fac n) * s (n + 1)) := by
    have h1 : n + 1 ≥ n + 1 := by algebra
    apply isInt_fac_mul_s (n+1) n h1
  have hsame : (fac n) * (e - s (n+1)) = (fac n) * e - (fac n) * s (n+1) := by algebra
